"""Wild mixture of the implementations listed below.
...
from otx.algo.diffusion.model.ldm.models.autoencoder import (
    AutoencoderKL,
    IdentityFirstStage,
    VQModelInterface,
)
from otx.algo.diffusion.model.ldm.models.diffusion.ddim import DDIMSampler
from otx.algo.diffusion.model.ldm.modules.diffusionmodules.util import (
    extract_into_tensor,
    make_beta_schedule,
    noise_like,
)
from otx.algo.diffusion.model.ldm.modules.distributions.distributions import (
    DiagonalGaussianDistribution,
    normal_kl,
)
from otx.algo.diffusion.model.ldm.modules.ema import LitEma
from otx.algo.diffusion.model.ldm.util import (
    count_params,
    default,
    exists,
...
def uniform_on_device(
    r1: float, r2: float, shape: tuple[int], device: str
) -> torch.Tensor:
    """Generate uniform tensor on device."""
    return (r1 - r2) * torch.rand(*shape, device=device) + r2


class OTXDDPM(OTXModel[DiffusionBatchDataEntity, DiffusionBatchPredEntity]):
    ...
    def __init__(  # noqa: PLR0913
        ...
        **otxmodel_kwargs,
        ...
        if label_info != 0:
            msg = "label_info must be 0"
            raise ValueError(msg)
        super().__init__(label_info, **otxmodel_kwargs)
        if parameterization not in ["eps", "x0"]:
            msg = 'currently only supporting "eps" and "x0"'
            raise ValueError(msg)
    ...
    def get_input(
        ...
        if isinstance(x, list):
            if len(x) and isinstance(x[0], str):
                ...
                batch.images, batch.imgs_info = stack_batch(
                    batch.images,
                    batch.imgs_info,
...
class OTXLatentDiffusion(OTXDDPM):
    """main class"""
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["captions", "coordinates_bbox"]:
                    xc = getattr(batch, cond_key)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if (
                self.cond_stage_key
                in ["images", "LR_image", "segmentation", "bbox_img"]
        ...
        **kwargs,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
            elif self.cond_stage_key in ["captions"]:
...
class Layout2ImgDiffusion(OTXLatentDiffusion):
